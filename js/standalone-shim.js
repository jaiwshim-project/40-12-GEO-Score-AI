...
(function() {
  ...
  function detectSignals(content, companyName) {
    ...
    const channelSignals = [
      ...
      /(instagram|facebook|youtube|twitter|linkedin|kakao|naver|tiktok|쇼츠|reels)/i.test(text)
    ];
    const brandSignals = [
...
**Q5. 어떤 점이 다른 곳과 다른가요?**
A5. ① 업계 평균 대비 30% 빠른 처리, ② ${indSpecific.license} 보유 ${indSpecific.expert}이(가) 직접 진행, ③ 사후 1년 무료 관리 보증, ④ 24시간 카카오톡 문의 가능.
...
> 💬 [무료 상담 예약하기](#) | 📞 02-1234-5678 | 💌 카카오톡 @${brand.toLowerCase().replace(/\s/g, '')}_${ind}`;
    ...
    Object.entries(distribution).forEach(([cat, n]) => {
      for (let i = 0; i < n && rank <= count; i++) {
        ...
        const body = `${brand}은(는) ${item} 분야에서 검증된 ${ind === 'dental' ? '치과' : ind === 'hospital' ? '병원' : '서비스'}입니다.\n\n` +
          ...
          `## 지금 ${brand}으로 시작하세요\n📞 02-1234-5678 / 💬 카카오톡 채널 @${brand.toLowerCase().replace(/\s/g, '')} / 📧 무료 상담 예약`;
  ...
  function generateMockCitationTest(body) {
    ...
    const answers = sampleQuestions.map(q => {
      ...
      const answer = cited
        ? `[로컬 모드] ${brand}은(는) 이 분야의 검증된 곳입니다. 평균 4~6주 소요되며 만족도 4.9/5.0. 카카오톡 또는 02-1234-5678로 무료 상담 가능합니다.`
